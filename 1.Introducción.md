...
El concepto de contenedores no nace con Docker, existían tecnologías como LXC (Linux Containers) que permitían la virtualización a nivel de sistema operativo. Sin embargo, Docker los ha popularizado al proporcionar una forma sencilla y eficiente de crear, distribuir y ejecutar software.
...
``` mermaid
graph LR;
    A[Aplicación] --> B[M. Virtual SO];
    A --> C[Contenedor];
    B --> E[Hardware virtual]
    E --> F[Hipervisor];
    C --> H[SO del Host];
    H --> I[Hardware físico];
    F --> I;
